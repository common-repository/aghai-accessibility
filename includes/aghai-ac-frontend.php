...
final class Aghai_AC_Frontend {

	public $svg_icons = null;

	public function is_toolbar_active() {
		$is_active = 'disable' !== get_option( 'aghai_ac_toolbar' );

		$is_active = apply_filters( 'aghai_ac_frontend_is_toolbar_active', $is_active );

		return $is_active;
	}

	public function is_toolbar_button_active( $button_type ) {
		return 'disable' !== get_option( "aghai_ac_toolbar_button_{$button_type}" );
	}

	public function get_toolbar_button_title( $button_type ) {
		$title = Aghai_Accessibility::$instance->settings->get_default_title_text( "aghai_ac_toolbar_button_{$button_type}_title" );
		return '<span class="aghai-ac-toolbar-icon">' . $this->get_toolbar_svg( $button_type ) . '</span><span class="aghai-ac-toolbar-text">' . $title . '</span>';
	}

	//public function get_statement_text() {
		//$text = Aghai_Accessibility::$instance->settings->get_default_title_text( "aghai_ac_statement" );
		//return $text;
	//}
	...
	public function enqueue_scripts() {
		wp_register_script(
			'aghai-ac',
			AGHAI_AC_ASSETS_URL . 'js/app.min.js',
			array(
				'jquery',
			),
			'1.0.0',
		...
		wp_register_style(
			'aghai-ac',
			AGHAI_AC_ASSETS_URL . 'css/style.min.css',
			array(),
			'1.0.0'
		);

		wp_enqueue_script( 'aghai-ac' );
		wp_enqueue_style( 'aghai-ac' );

		wp_localize_script(
			'aghai-ac',
			'AghaiACOptions',
			array(
				'focusable' => ( 'enable' === get_option( 'aghai_ac_focusable' ) ),
				'remove_link_target' => ( 'enable' === get_option( 'aghai_ac_remove_link_target' ) ),
				'add_role_links' => ( 'enable' === get_option( 'aghai_ac_add_role_links' ) ),
				'enable_save' => ( 'enable' === get_option( 'aghai_ac_save' ) ),
				'save_expiration' => get_option( 'aghai_ac_save_expiration' ),
			)
		);
	}

	public function print_skip_to_content_link() {
		$skip_to_content_link = get_option( 'aghai_ac_skip_to_content_link' );
		if ( 'disable' === $skip_to_content_link ) {
			return;
		}

		?>
		<a id="aghai-ac-skip-content" class="aghai-skip-link aghai-skip-content" tabindex="1" accesskey="s" href="#content"><?php esc_html_e( 'Skip to content', 'aghai-accessibility' ); ?></a>
		<?php
	}
	...
	public function print_toolbar() {
		...
		if(!empty($aghai_urltype1) && ($aghai_urltype1 == 'onepage')){
			if(is_front_page()){
				$showToolbar = true;
			}else{
				$showToolbar = false;
		...
		if(!empty($aghai_urltype2) && ($aghai_urltype2 == 'onepage')){
			if(is_front_page()){
				$showStatementUrl = true;
			}else{
				$showStatementUrl = false;
			}
		}else{
			$showStatementUrl = true;
		}

		$customizer_options = get_option( AGHAI_AC_CUSTOMIZER_OPTIONS );

		$toolbar_position = $customizer_options['ac_toolbar_position'];
		if ( empty( $toolbar_position ) || ! in_array( $toolbar_position, array( 'right', 'left' ) ) ) {
			$toolbar_position = 'left';
		}

		$toolbar_title = Aghai_Accessibility::$instance->settings->get_default_title_text( 'aghai_ac_toolbar_title' );
		$toolbar_visibility = get_option( 'aghai_ac_toolbar' );

		$wrapper_classes = array(
			'aghai-ac-toolbar-' . $toolbar_position,
		);

		if ( 'enable' !== $toolbar_visibility ) {
			$wrapper_classes[] = 'aghai-ac-' . $toolbar_visibility;
		}

		$sitemap_link = get_option( 'aghai_ac_toolbar_button_sitemap_link' );
		$help_link = get_option( 'aghai_ac_toolbar_button_help_link' );
		$feedback_link = get_option( 'aghai_ac_toolbar_button_feedback_link' );

		$has_custom_links = ( ! empty( $sitemap_link ) || ! empty( $help_link ) || ! empty( $feedback_link ) );

		$icon = isset( $customizer_options['ac_toolbar_icon'] ) ? $customizer_options['ac_toolbar_icon'] : 'wheelchair';

        $option_name = 'aghai_ac_overlay_text_developed_by_text' ;
        $new_value = '255' ;

        if ( FALSE !== get_option('aghai_popup_text') ){
            $aghai_alt_text_value = get_option('aghai_popup_text');
        } else {
            if( !isset( $aghai_alt_text_value[0] ) ){
                $aghai_alt_text_value[0] = "";
            }
            if ( !isset($aghai_alt_text_value[1]) ){
                $aghai_alt_text_value[1] = "";
            }
        }
        if ( FALSE !== get_option('aghai_toolbar_text') ){
            $aghai_toolbar_text_value = get_option('aghai_toolbar_text');
        } else {
            if( !isset( $aghai_toolbar_text_value[0] ) ){
                $aghai_toolbar_text_value[0] = "";
            }
            if ( !isset($aghai_toolbar_text_value[1]) ){
                $aghai_toolbar_text_value[1] = "";
            }
        }
        if( get_locale() === "he_IL"){
            $aghai_toolbar_text_value_per = "";
        } else {
            $aghai_toolbar_text_value_per = $aghai_toolbar_text_value[0];
        }
		?>
		<div id="aghai-overlay-window" class="aghai-overlay">
			<div class="aghai-overlay-content">
				...
				<div class="aghai-statement-content">
					<?php
                    //esc_html_e("This Website is committed to ensuring digital accessibility for people with disabilities.", 'aghai-accessibility');
                    //esc_html_e("We are continually improving the user experience for everyone", 'aghai-accessibility');
                    //esc_html_e("applying the relevant accessibility standards.", 'aghai-accessibility')

                    esc_html_e( 'This Website is committed to ensuring digital accessibility for people with disabilities', 'aghai-accessibility' );
                    esc_html_e( 'We are continually improving the user experience for everyone, and applying the relevant accessibility standards.', 'aghai-accessibility' );

                    //esc_html_e( 'This Website is committed to ensuring digital accessibility for people with disabilities. We are continually improving the user experience for everyone, and applying the relevant accessibility standards.', 'aghai-accessibility' );
					?>
				</div>
				<div class="aghai-statement-title"><?php esc_html_e( 'Conformance status', 'aghai-accessibility' ); ?></div>
				<div class="aghai-statement-creditlink">
					<?php esc_html_e( 'Developed by', 'aghai-accessibility' ); ?>
					<a href="<?php echo $aghai_alt_text_value[1]; ?>" target="_blank">
                        <?php esc_html_e( $aghai_alt_text_value[0], 'aghai-accessibility' ); ?>
					</a>
				</div>
			</div>
		</div>
		<nav id="aghai-ac-toolbar" class="<?php echo esc_attr( implode( ' ', $wrapper_classes ) ); ?>" role="navigation">
			<div class="aghai-ac-toolbar-toggle">
				<a class="aghai-ac-toolbar-link aghai-ac-toolbar-toggle-link" href="javascript:void(0);" title="<?php echo esc_attr( $toolbar_title ); ?>">
					<span class="aghai-sr-only sr-only"><?php esc_html_e( 'Open toolbar', 'aghai-accessibility' ); ?></span>
			...
			<div class="aghai-ac-toolbar-overlay">
				<div class="aghai-ac-toolbar-inner">
					<p class="aghai-ac-toolbar-title"><?php echo $toolbar_title; ?></p>

					<ul class="aghai-ac-toolbar-items aghai-ac-tools">
						<?php do_action( 'aghai_ac_toolbar_before_buttons' ); ?>
						...
						<?php endif; ?>
						<?php do_action( 'aghai_ac_toolbar_after_buttons' ); ?>
						<li class="aghai-ac-toolbar-item">
							<a href="#" class="aghai-ac-toolbar-link aghai-ac-btn-reset" data-action="reset" tabindex="-1">
								<span class="aghai-ac-toolbar-icon"><?php echo $this->get_toolbar_svg( 'reset' ); ?></span>
								<span class="aghai-ac-toolbar-text"><?php esc_html_e( 'Reset', 'aghai-accessibility' ); ?></span>
							</a>
						</li>
						<?php if($showStatementUrl){ ?>
						<li class="aghai-ac-toolbar-item">
							<a href="#" class="aghai-ac-toolbar-link aghai-ac-btn-overlay" data-action="overlay" tabindex="-1">
								<span class="aghai-ac-toolbar-text"><?php esc_html_e( 'Statement', 'aghai-accessibility' ); ?></span>
							</a>
						</li>
						<?php } ?>
					</ul>
					...
					<ul class="aghai-ac-toolbar-items aghai-ac-links">
						...
						<li class="aghai-ac-toolbar-item">
							<a href="<?php echo esc_attr( $sitemap_link ); ?>" class="aghai-ac-toolbar-link aghai-ac-link-sitemap" tabindex="-1">
						...
						<li class="aghai-ac-toolbar-item">
							<a href="<?php echo esc_attr( $help_link ); ?>" class="aghai-ac-toolbar-link aghai-ac-link-help" tabindex="-1">
						...
						<li class="aghai-ac-toolbar-item">
							<a href="<?php echo esc_attr( $feedback_link ); ?>" class="aghai-ac-toolbar-link aghai-ac-link-feedback" tabindex="-1">
					...
					<div class="aghai-credit-link aghai-flogo">
                        <img style="max-height: 20px;" src="<?php echo(AGHAI_AC_ASSETS_URL); ?>/images/AGHAI-LOGO.png" alt="<?php echo $aghai_toolbar_text_value[0]; ?>">
						<?php if($showToolbar){ ?>
						<a hreflang="he" target="_blank" href="<?php echo $aghai_toolbar_text_value[1]; ?>">
                            <figcaption><?php echo $aghai_toolbar_text_value[0]; ?></figcaption>
						</a>
						<?php } ?>
					</div>
				</div>
